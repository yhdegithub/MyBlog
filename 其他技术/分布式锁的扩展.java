做项目，使用到了分布式锁，参考博客做了一些实现，但是总觉得不全面，看完了zookeeper终于可以写的出来一点东西了。目前分布式锁主要有两种实现方式 ：
基于redis实现的 ， 基于zookeeper ,还有很少用到的基于数据库乐观锁的 。 一个良好的分布式锁，必须同一时刻只能最多有一个用户使用 ，用户使用完之
后释放锁，释放的也是自己的锁，不是其他人的（主要是因为redis设置的超时时间的问题）
...
2）使用zookeeper
   Zookeeper就是一个提供分布式一致性服务的东西儿，很多东西都给我们封装了。 我们先在一个主机下创建一个永久节点ParentLock    
...
由于Client2一直监听着Lock1的存在状态，当Lock1节点被删除，Client2会立刻收到通知。这时候Client2会再次查询ParentLock下面的所有节点，确认自己
创建的节点Lock2是不是目前最小的节点。如果是最小，则Client2顺理成章获得了锁。同理，如果Client2也因为任务完成或者节点崩溃而删除了节点Lock2，
那么Cient3就会接到通知
  

3) 数据库乐观锁 。
    这个使用原理就是插入一个具有唯一性约束的数据行 ，只能有一个用户成功插入，其他的都失败，故成功插入的才能拿到锁 ； 
    释放锁可以直接用删除但是，在大量用户时，这种压力全都给后端数据库了 ，这种方式很少使用吧
